...
class ControllerBase {
  constructor(socket, options = {}) {
    ...
    this._onVideoUpdateStartedSignalBound = this._onVideoUpdateStartedSignal.bind(
      this
    )
    this._onVideoUpdateEndedSignalBound = this._onVideoUpdateEndedSignal.bind(
      this
    )
    this._onEndedSignalBound = this._onEndedSignal.bind(this)
    this._onTimeupdateSignalBound = this._onTimeupdateSignal.bind(
      this
    )
    this._onVideoPlayingSignalBound = this._onVideoPlayingSignal.bind(
      this
    )
    this._onVideoPausedSignalBound = this._onVideoPausedSignal.bind(
      this
    )
    this._onVideoWaitingSignalBound = this._onVideoWaitingSignal.bind(
      this
    )
    ...
    emitter.on(
      `${options.id}:controller:video:previous:video`,
      this._previousVideoBound
    )
    emitter.on(
      `${options.id}:controller:video:next:video`,
      this._nextVideoBound
    )
    emitter.on(
      `${options.id}:controller:video:previous:segment`,
      this._previousSegmentBound
    )
    emitter.on(
      `${options.id}:controller:video:next:segment`,
      this._nextSegmentBound
    )
  ...
  _initListeners() {
    this.mediaSources.forEach(ms => {
      ...
      ms.videoUpdateStartedSignal.add(
        this._onVideoUpdateStartedSignalBound
      )
      ms.videoUpdateEndedSignal.add(
        this._onVideoUpdateEndedSignalBound
      )
